...
async function main() {
  ...
  // const statement = await verifier.verifyTx()
  
  console.log('Verifier deployed to:', verifier.target);

  // const zokContract = new ethers.Contract(verifier.target, abi, provider2);
  // const connectContract = zokContract.connect(hrdhatAccountPublic)
  // const tx = await connectContract.verifyTx(proof.proof, proof.inputs)

  const x = await verifier.connect(address1).verifyTx(proof.proof, proof.inputs);
