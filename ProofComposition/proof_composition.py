...
def verifyProofs(paths, dir_path):
    for path in paths:
        result = subprocess.run(["zokrates", "verify", "-j", f"{dir_path}/{path}/proof.json", "-v", f"{dir_path}/{path}/verification.key"], stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)
...
def aggregate_proofs(paths, dir_path):
    return verifyProofs(paths, dir_path)
