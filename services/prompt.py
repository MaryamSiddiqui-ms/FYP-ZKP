...
prompt_parts = [
  ...
  "input: input = x_test[20]input = np.array(input)input = input.reshape(28,28,1)output_1 = conv2d(input, filters_1, bias_1, filters_1.shape[0])activated_1 = relu(output_1)pooled_1 = maxPool_2d(activated_1, 2)output_2 = conv2d(pooled_1, filters_2, bias_2, filters_2.shape[0])activated_2 = relu(output_2)pooled_2 = maxPool_2d(output_2, 2)output_3 = conv2d(pooled_2, filters_3, bias_3, filters_3.shape[0])activated_3 = relu(output_3)flattened_layer = flatten(activated_3).reshape(-1, 1)output_d1 = apply_weights(dense_1, flattened_layer, bias_d1)output_d1.reshape(1,-1)activated_d1 = relu(output_d1)output_d2 = apply_weights(dense_2, output_d1, bias_d2)activated_d2 = relu(output_d2).reshape(-1)final = softmax(activated_d2)prediction = argmax(final)print(prediction)",
  "output: ```python\ninput = x_test[20]\ninput = np.array(input)\ninput = input.reshape(28,28,1)\n# Checkpoint #1: Convolution\n# zkConv(input, filters_1, bias_1, filters_1.shape[0]) -> output_1, proofOfConv\noutput_1 = conv2d(input, filters_1, bias_1, filters_1.shape[0])\n# Checkpoint #2: ReLU\n# zkRelu(output_1) -> activated_1, proofOfRelu\nactivated_1 = relu(output_1)\n# Checkpoint #3: MaxPooling\n# zkMaxPool(activated_1, 2) -> pooled_1, proofOfMaxPool\npooled_1 = maxPool_2d(activated_1, 2)\n# Checkpoint #4: Convolution\n# zkConv(pooled_1, filters_2, bias_2, filters_2.shape[0]) -> output_2, proofOfConv\noutput_2 = conv2d(pooled_1, filters_2, bias_2, filters_2.shape[0])\n# Checkpoint #5: ReLU\n# zkRelu(output_2) -> activated_2, proofOfRelu\nactivated_2 = relu(output_2)\n# Checkpoint #6: MaxPooling\n# zkMaxPool(output_2, 2) -> pooled_2, proofOfMaxPool\npooled_2 = maxPool_2d(output_2, 2)\n# Checkpoint #7: Convolution\n# zkConv(pooled_2, filters_3, bias_3, filters_3.shape[0]) -> output_3, proofOfConv\noutput_3 = conv2d(pooled_2, filters_3, bias_3, filters_3.shape[0])\n# Checkpoint #8: ReLU\n# zkRelu(output_3) -> activated_3, proofOfRelu\nactivated_3 = relu(output_3)\nflattened_layer = flatten(activated_3).reshape(-1, 1)\n# Checkpoint #10: ApplyWeights\n# zkApplyWeights(dense_1, flattened_layer, bias_d1) -> output_d1, proofOfApplyWeights\noutput_d1 = apply_weights(dense_1, flattened_layer, bias_d1)\noutput_d1.reshape(1,-1)\n# Checkpoint #11: ReLU\n# zkRelu(output_d1) -> activated_d1, proofOfRelu\nactivated_d1 = relu(output_d1)\n# Checkpoint #12: ApplyWeights\n# zkApplyWeights(dense_2, output_d1, bias_d2) -> output_d2, proofOfApplyWeights\noutput_d2 = apply_weights(dense_2, output_d1, bias_d2)\n# Checkpoint #13: ReLU\n# zkRelu(output_d2) -> activated_d2, proofOfRelu\nactivated_d2 = relu(output_d2).reshape(-1)\n# Checkpoint #14: Softmax\n# zkSoftmax(activated_d2) -> final, proofOfSoftmax\nfinal = softmax(activated_d2)\n# Checkpoint #15: Argmax\n# zkArgmax(final) -> prediction, proofOfArgmax\nprediction = argmax(final)\nprint(prediction)\n```",
]
